import type { ProjectData, WeeklyData, PortfolioSeriesPoint } from '../types';
...
/**
 * Average baseline/actual across all projects per weekIndex → series for SCurveChart.
 * A series with no entries at a given weekIndex yields `null` (not 0), so the plan line
 * can span the full timeline while the actual line stops at the last real actual week
 * instead of dipping to zero for future/unrealised weeks.
 */
export const portfolioSeries = (projects: ProjectData[]): PortfolioSeriesPoint[] => {
